...
const SourceContentViewer = ({ 
  ...
  const getSourceIcon = (type: string) => {
    ...
    return iconMap[type] || '📄'; // fallback to document icon
  };

  // Split content into lines for highlighting or use excerpt if no full content
  const content = sourceContent || citation.excerpt || citation.document?.pageContent || '';
  ...
  return (
    <div className={`flex flex-col h-full overflow-hidden ${className}`}>
      ...
      <div className="p-4 border-b border-gray-200 flex-shrink-0">
        ...
        {(citation.address || citation.suburb) && (
          <div className="text-xs text-gray-500">
            {citation.address && citation.suburb ? `${citation.address}, ${citation.suburb}` : citation.address || citation.suburb}
          </div>
        )}
      </div>
